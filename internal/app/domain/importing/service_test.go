...
func (suite *ServiceSuite) Test_Success() {
	...
	suite.Equal(aggregator.ImportJobResultNew, dsl.ImportJob(uuid.NewSHA1(chID, []byte("bankkaufmann-fur-front-office-middle-office-back-office-munich-304839"))).Result)
	suite.Equal(aggregator.ImportJobResultNew, dsl.ImportJob(uuid.NewSHA1(chID, []byte("fund-accountant-wertpapierfonds-munich-310570"))).Result)
	...
	suite.Equal(aggregator.JobPublishStatusUnpublished, dsl.Job(jNew1ID).PublishStatus)
	...
	suite.Equal(aggregator.JobPublishStatusUnpublished, dsl.Job(jNew2ID).PublishStatus)
