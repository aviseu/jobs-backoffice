...
func (suite *GatewaySuite) Test_ImportChannel_Success() {
	...
	suite.NoError(err)
	suite.Len(jr.Jobs, 4)
	suite.Equal(aggregator.JobPublishStatusPublished, jr.Jobs[j1.ID()].PublishStatus)
	suite.Equal(aggregator.JobPublishStatusUnpublished, jr.Jobs[j2.ID()].PublishStatus)
